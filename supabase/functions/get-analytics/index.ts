...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    let emailQuery = supabase.from('test_results').select('email', {
      count: 'exact',
      head: true
    });
    ...
    let resultsQuery = supabase.from('test_results').select('*', {
      count: 'exact',
      head: true
    });
    ...
    let approvedQuery = supabase.from('payments').select('*', {
      count: 'exact',
      head: true
    }).eq('status', 'approved');
    ...
    // 5. Receita total
    let revenueQuery = supabase.from('payments').select('amount').eq('status', 'approved');
    ...
    for (const p of priceSegData || []) {
      ...
      if ((p.status || '').toLowerCase() === 'approved') {
        priceSegments[key].payments_approved += 1;
        priceSegments[key].revenue += Number(p.amount ?? 0);
      }
    }
    const conversionRate = resultsAccessed && resultsAccessed > 0 ? ((paymentsApproved || 0) / resultsAccessed * 100).toFixed(2) : '0.00';
    const response = {
      period: {
