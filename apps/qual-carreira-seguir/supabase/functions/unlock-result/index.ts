...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: result, error: resultError } = await supabase.from('test_results').select('id, is_unlocked, email').eq('id', resultId).single();
    ...
    // Special case: FREE_ payments (100% discount coupons - don't query MercadoPago)
    if (paymentId === 'DEV_BYPASS' || paymentId.startsWith('FREE_')) {
      ...
      console.log(`[unlock-result] Skipping MP verification for: ${paymentId}`);
    } else {
      console.log(`[unlock-result] Checking payment status for payment_id: ${paymentId}`);
      const { data: paymentData, error: paymentError } = await supabase.functions.invoke('check-payment-status', {
        body: {
